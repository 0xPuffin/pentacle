...
const findInGrandChildName = (data, childName, grandChildName) => {
  if (data && data.children) {
    const socialChild = data.children.find(
      (child) =>
        child.name && child.name.toLowerCase() === childName.toLowerCase()
    );
    // TODO: Simplify this
    // eslint-disable-next-line no-mixed-operators
    const child =
      (!!socialChild &&
        socialChild.children.find((child) =>
          child.name.toLowerCase().includes(grandChildName)
        )) ||
      data.children.find((child) =>
        child.name.toLowerCase().includes(grandChildName)
      );
    if (child) {
      return child;
  ...
  return function define(runtime, observer) {
    ...
    main
      ...
      .define(
        ...
        function (partition, data, d3, width, color, arc, format, radius) {
          ...
          const circle = label.append("svg").attr("r", 15);
          ...
          label
            ...
            .attr(
              "font-family",
              "'matter', arial, 'Helvetica-neue', Helvetica, sans-serif"
            )
            .attr("color", "white")
            .attr("dy", "0.35em")
            .on("click", clicked)
          ...
          const twitterEl = svg
            .append("text")
            .attr("style", "font-size: .875rem; fill: white; text-align: left;")
            .attr("x", "0%")
            .attr("y", "1%")
            .on("click", (d) => {
              if (d && d.target && d.target.textContent) {
                window.open(
                  `https://www.twitter.com/${d.target.textContent}`,
                  "_blank"
                );
              }
            });
    ...
    main
      ...
      .define("data", ["FileAttachment"], function (FileAttachment) {
        return FileAttachment(`${name}.json`).json();
      });
    main
    ...
    main
      ...
      .define("color", ["d3", "data"], function (d3, data) {
        ...
        return d3.scaleOrdinal(
          d3.quantize(d3[colorScheme], data.children.length + 1)
        );
      });
