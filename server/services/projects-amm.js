...
async function getMultiple(page = 1) {
    ...
    const rows = await db.query(
        ...
        'SELECT project.project_id, project.project_name, project.description, tag.tag_name FROM project JOIN project_tag ON project.project_id = project_tag.project_id JOIN tag ON tag.tag_id = project_tag.tag_id WHERE tag.tag_id = 1 OFFSET $1 LIMIT $2',    // tag_id 1 is amm, hardcorded zz
        [offset, config.listPerPage]
    );
    const data = helper.emptyOrRows(rows);
    const meta = {page};
